	     ...
	     derivs *u)
{
  ...
  */
  ...
  int i, j, k, ivar, indx;
  double al, be, A, B, T, R, x, phi, y, z, Am1, *values;
  derivs *U;
      ...
      for (j = 0; j < n2; j++)
	{
	  for (k = 0; k < n3; k++)
	      ...
	      for (ivar = 0; ivar < nvar; ivar++)
		{
	      ...
	      /* Calculation of (y,z) and*/
	      /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
	      RT3_To_xyz (nvar, R, T, phi,&x, &y, &z, U);

	      NonLinEquations (sources[Index(0,i,j,k,1,n1,n2,n3)],
			       A, B, T, R, x , phi, y, z, U, values);
		 ...
		 double *Jdv, derivs *u)
{
  ...
  */
  ...
  int i, j, k, ivar, indx;
  double al, be, A, B, R, T, x, phi, y, z, Am1, *values;
  derivs *dU, *U;

  Derivatives_AB3 (nvar, n1, n2, n3, dv);

#ifdef TP_OMP
#pragma omp parallel for private (values,dU,U,i,j,k,al,A,be,B,phi,T,R,x,y,z,Am1,ivar,indx) schedule(dynamic)
#endif
  for (i = 0; i < n1; i++)
    {
      values = dvector (0, nvar - 1);
      ...
      for (j = 0; j < n2; j++)
	{
	  for (k = 0; k < n3; k++)
	      ...
	      for (ivar = 0; ivar < nvar; ivar++)
		{
	      ...
	      RT3_To_xyz (nvar, R, T, phi, &x, &y, &z, dU);

	      LinEquations (A, B, R, T, x, phi, y, z, dU, U, values);
	      for (ivar = 0; ivar < nvar; ivar++)
		{
		  indx = Index (ivar, i, j, k, nvar, n1, n2, n3);
		   ...
		   int n3, derivs *dv, derivs *u, double *values)
{
  ...
  int ivar, indx;
  double al, be, A, B, R, T, x, phi, y, z, Am1;
					 ...
					 double x, double y, double z)
{
  ...
  double phi, T, r1, A, B, al, be, a, b, c,
    result, Ui;
  int i, j, k;
  ...
  /* MR: all foregoing codes are for calculating the A and B in terms of xyz */
				       ...
				       double x, double y, double z)
{
  ...
  double phi, r1, T, R, A, B, result, Ui;
...
void set_initial_guess(derivs *v)
{
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++) {
          ...
          /* Calculation of (X,R)*/
          AB_To_RT (nvar, A, B, &T, &R, U);

	  /* Calculation of (x,r)*/
      //    C_To_c (nvar, X, R, &(s_x[i3D]), &r, par_b, U);

	  /* Calculation of (y,z)*/
          RT3_To_xyz (nvar, R, T, phi, &s_x[i3D], &(s_y[i3D]), &(s_z[i3D]), U);
  ...
  if (params_get_int("do_initial_debug_output")) {
	...
	for (j = 0; j < n2; j++) {
	  ...
	  /* Calculation of (y,z)*/
	  RT3_To_xyz (nvar, R, T, phi, &s_x[i3D], &(s_y[indx]), &(s_z[indx]), U);
	  fprintf(debug_file,
    ...
    fprintf(debug_file, "\n\n");
    /*  I did not undrestand it so i remove it :)))))
